...
class FastVolumeMonitor(FastMonitor):
    ...
    def emit(self,venue,symbol,asset,evidence,bid,ask,ts,scan_ts,rank):
        ...
        if reason:self.audit.record('ALERT_SUPPRESSED',ident,venue,symbol,'ALERT_ONLY',ts_ms=ts,reason=reason)
            ...
            try:self.events.put_nowait({'id':ident,'text':message,'ts_ms':ts})
    ...
    def observe(self,venue):
        ...
        while not self.stop.is_set():
            if self.rule_version=='fast-price-rise-v4' and not self.control()['enabled']:
                with self.lock:
                    self.observations={k:v for k,v in self.observations.items() if v['venue']!=venue}
                    self.tracking={}
                self.stop.wait(1)
                continue
            started=now()
