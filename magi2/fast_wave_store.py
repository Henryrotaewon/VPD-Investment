...
class EvidenceStore:
    ...
    def record_confirmation(self, point, result):
        with self.db:
            self.db.execute('CREATE TABLE IF NOT EXISTS confirmations ('
                            'venue TEXT, symbol TEXT, observed_ms INTEGER, payload TEXT NOT NULL, '
                            'PRIMARY KEY(venue,symbol,observed_ms))')
            self.db.execute('INSERT OR IGNORE INTO confirmations VALUES(?,?,?,?)',
                            (point['venue'],point['symbol'],point['observed_ms'],json.dumps(result,allow_nan=False)))

    def prune(self, now_ms):
        # Filled trade evidence and the paper daily journal are retained separately.
        with self.db:
            self.db.execute('DELETE FROM observations WHERE observed_ms<?',(now_ms-2*86400000,))
            self.db.execute('DELETE FROM evaluations WHERE observed_ms<?',(now_ms-8*86400000,))
            self.db.execute('DELETE FROM errors WHERE observed_ms<?',(now_ms-8*86400000,))
            self.db.execute('CREATE TABLE IF NOT EXISTS confirmations ('
                            'venue TEXT, symbol TEXT, observed_ms INTEGER, payload TEXT NOT NULL, '
                            'PRIMARY KEY(venue,symbol,observed_ms))')
            self.db.execute('DELETE FROM confirmations WHERE observed_ms<?',(now_ms-8*86400000,))
