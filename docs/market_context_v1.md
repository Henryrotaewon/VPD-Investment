...
종목 JSON은 공백 없는 UTF-8 리터럴로 직렬화한다(ensure_ascii=False). 비영문 종목을 ASCII \u 이스케이프로 보내면 Binance의 symbols 정규식 검사에서 -1100 오류가 발생할 수 있다. 100개 초과 목록·비영문 종목에 대한 회귀 검사를 포함한다.
