...
Current selection is `fast-volume-accel-v2`: each minute inspect the strongest five-minute price gainers (maximum 20 detailed probes), require completed five-minute gain >=1%, greater than the preceding five-minute gain, and quote turnover >=2x the preceding five minutes. No spread/tick, 30-second breakout, chase or buyer-share gate is used in v2. Initial price warmup, failed/missing candles and probe-budget exclusions are explicit. All signals reach paper accounting before notification throttling. See `docs/FAST_PAPER.md` for the complete active policy. This service does NOT submit real orders or restart Shadow.
...
Legacy v1 forward samples used the 15-minute candidate window. V2 uses independent ten-minute observations after detection, capped at 50 per venue, and independent paper-exit workers. Restart loss, gaps, observation-cap overflow and missing samples are unavailable. Evaluation version `forward-5m-bid-v1` remains separate from selection rules. `/fast_compare` separates v1 and v2; this is a before/after comparison, not concurrent A/B evidence.
...
Historical v1: at a price breakout, a bounded recent-trade sample measured buyer-taker quote share, requiring >=20 trades, >=5s span, latest <=3s, share >=70%, spread <=10bps and breakout >=10bps. This sampled 100-point buy-share measure was not a probability. Existing records retain their definitions. V2 displays turnover multiples and price acceleration, never an invented buyer-share score.

Append-only ALERT_RESERVED / ALERT_SUPPRESSED events retain cross-venue same-asset once/hour (XBT normalized BTC), global <=3 per rolling hour and >=10min between notifications. Reservations count even if delivery fails. Suppression does not remove captured evidence or paper eligibility. Historical rules remain `fast-auto-v1` / `fast-buy-flow-v1`; new selection is `fast-volume-accel-v2`.
